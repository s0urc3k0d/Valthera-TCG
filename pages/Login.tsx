...
export const Login: React.FC = () => {
  ...
  const handleLogin = async () => {
    // Utiliser la redirection (plus fiable que popup)
    await login();
  };
      ...
      <div className="w-full max-w-md bg-valthera-800 p-8 rounded-2xl shadow-2xl border border-valthera-700">
          ...
          <div className="flex justify-center mb-4">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" className="w-16 h-16">
              <defs>
                ...
                <linearGradient id="loginInner" x1="0%" y1="0%" x2="0%" y2="100%">
                  <stop offset="0%" style={{stopColor:'#2D1B0E'}} />
                  <stop offset="100%" style={{stopColor:'#1A0F08'}} />
                </linearGradient>
              </defs>
              <path d="M50 5 L90 20 L90 50 Q90 80 50 95 Q10 80 10 50 L10 20 Z" fill="url(#loginShield)" stroke="#5C4A1F" strokeWidth="2"/>
              <path d="M50 12 L82 24 L82 48 Q82 74 50 87 Q18 74 18 48 L18 24 Z" fill="url(#loginInner)"/>
              <path d="M35 30 L50 70 L65 30 L58 30 L50 55 L42 30 Z" fill="url(#loginShield)"/>
            </svg>
          </div>
          <h2 className="text-3xl font-medieval font-bold text-valthera-200">Bienvenue, Aventurier</h2>
          <p className="text-valthera-500 text-sm mt-2">Connectez-vous ou créez un compte</p>
        </div>
        ...
        <div className="mt-8 p-4 bg-valthera-900/50 rounded-lg border border-valthera-700/50">
          <p className="text-valthera-400 text-sm text-center">
            🛡️ Connexion sécurisée via <span className="text-valthera-300">Auth0</span>
          </p>
          <p className="text-valthera-500 text-xs text-center mt-2">
            Vous serez redirigé vers la page de connexion Auth0.<br/>
            Créez un compte avec <strong>email + mot de passe</strong> ou connectez-vous.
          </p>
        </div>
        
        <div className="mt-6 text-center text-sm text-valthera-500">
          <Link to="/" className="text-valthera-400 hover:text-valthera-300 underline">
            ← Retour à l'accueil
          </Link>
        </div>
      </div>
